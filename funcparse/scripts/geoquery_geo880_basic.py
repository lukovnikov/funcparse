...
class GeoQueryDataset(object):
    def __init__(self,
        ...
        self.query_encoder = SentenceEncoder(tokenizer=basic_query_tokenizer)
...
def do_rare_stats(ds:GeoQueryDataset):
    ...
    def get_rare_portions(examples:List[BasicState]):
        ...
        for example in examples:
            total += 1
            question_tokens = example.inp_tokens
            query_tokens = example.gold_tokens
            both = True
            either = False
...
def run(lr=0.001,
    ...
    freedecoder = TFTokenSeqDecoder(tfdecoder.model)
    # # test
